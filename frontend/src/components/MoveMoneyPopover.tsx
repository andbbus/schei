...
export default function MoveMoneyPopover({
  ...
  return (
    <div
      ...
      className="absolute top-7 right-0 z-10 flex w-[296px] flex-col gap-3 rounded-[10px] border border-slate-200 bg-panel p-4 text-left shadow-[var(--elev-popover)] animate-[pop-in_180ms_cubic-bezier(0.2,0,0,1)]"
    >
      ...
      <input
        ...
        className="tnum mb-2 w-full rounded-md border border-slate-300 bg-panel px-2.5 py-1.5 text-sm text-slate-800 transition-colors placeholder:text-slate-400 focus:border-blue-500 focus:outline-none focus:ring-[3px] focus:ring-blue-500/35"
      />
      <select
        value={toId}
        onChange={(e) => setToId(e.target.value)}
        className="mb-2 w-full rounded-md border border-slate-300 bg-panel px-2.5 py-1.5 text-sm text-slate-800 transition-colors focus:border-blue-500 focus:outline-none focus:ring-[3px] focus:ring-blue-500/35"
      >
      ...
      <div className="flex gap-1">
        <button onClick={submit} className="flex-1 rounded bg-positive px-2 py-1 text-sm text-white">
          Move
        </button>
