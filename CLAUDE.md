...
- **Rebindable keyboard shortcuts**: `frontend/src/shortcuts.ts` is the pure core (tested) — action catalog, `bindingKey` normalization (`mod` = ⌘/Ctrl, letters keep explicit shift, punctuation like `?` doesn't), `formatBinding`, localStorage persistence (`schei.shortcuts.v1`). `App.tsx` runs the single keydown dispatcher: matched actions broadcast window events (`schei:palette`, `schei:add-txn`, `schei:shortcuts`) or navigate; plain-key bindings never fire while typing in a field (`allowInInput` only for the palette). Defaults: ⌘K palette, `n` add transaction, ⌘Z undo, `?` shortcut editor (`ShortcutsModal`, capture-phase listener with conflict detection + per-binding ✕/reset). The palette listens for `schei:palette` instead of hardcoding ⌘K.
- **Centered creation dialogs** (shared `Modal.tsx` shell — backdrop/Esc close): `TransactionModal` (global with account picker — sidebar "+ Add transaction", shortcut `n`, palette; or account-locked from the register's "+ Add Transaction" — keeps the repeat→scheduled branch and the category-learning hook via `onSaved`), `AccountModal` (replaced the old inline sidebar form), BudgetView `NameDialog` (category/group create + group rename — replaced `window.prompt`). `GET /budget` now returns `inflowCategoryId` so the transaction modal can offer "Inflow: Ready to Assign" (the /categories route excludes it).
